...
const FINANCE_EMPTY_RESET_VERSION = "20260628-finance-full-empty-v4-clean-cloud";
...
// Payroll advances must persist; finance empty reset must not clear payroll advance records.
...
function clearFinanceAndAdvanceLocalDataOnce() {
  try {
    ...
    for (let index = 0; index < localStorage.length; index += 1) {
      ...
      const isFinanceSettingsData = key === "nawah-finance-settings" || key.startsWith("nawah-finance-settings-");
      if (isFinanceDayData || isFinanceSettingsData) keysToRemove.push(key);
    }
...
async function clearEmployeeAdvanceFieldsOnce() {
  try {
    // لا يتم تصفير السلفيات عند تحديث الصفحة؛ هذا الروتين أصبح خاصًا بتثبيت علامة الترحيل فقط.
    ...
    console.warn("تعذر تثبيت إعداد حماية السلفيات من الحذف.", error);
  }
}
...
function applyCloudState(state) {
  ...
  if (localStorage.getItem(FINANCE_EMPTY_RESET_KEY) === FINANCE_EMPTY_RESET_VERSION) {
    delete state.financeSettings;
    delete state.financeDailyOpen;
    delete state.financeDailyDays;
    delete state.financeDailyCurrentDate;
    localStorage.setItem("nawah-finance-settings", JSON.stringify({ openingAmount: 0, custodyAmount: 0, updatedAt: "" }));
    localStorage.removeItem("nawah-finance-daily-open");
    localStorage.removeItem("nawah-finance-daily-days");
  }
...
(function precisionMinuteDocumentPatch(){
  ...
  function commitFinanceClosedSnapshots(totals, nextKey) {
    financeDaily.isClosed = true;
    financeDaily.closedAt = new Date().toISOString();
    financeDaily.nextFinanceDate = nextKey;
    financeDaily.carriedAmountSnapshot = totals.settings.openingAmount;
  ...
  function buildFinanceNextDay(nextKey, carriedAmount, pendingRows) {
    return normalizeFinanceDaily({
      financeDate: nextKey,
      pending: normalizeRows(pendingRows),
      deletedPending: [],
  ...
  function handleFinanceCloseDay() {
    ...
    const nextKey = getNextFinanceBusinessDate(financeDateKey);
    const nextParts = getArabicDateParts(nextKey);
    const confirmed = window.confirm(`سيتم إغلاق يوم ${currentParts.dayName} ${currentParts.dateLabel} وترحيل مبلغ ${moneyText(totals.newCarriedAmount)} إلى يوم ${nextParts.dayName} ${nextParts.dateLabel}. هل تريد المتابعة؟`);
    if (!confirmed) return;

    commitFinanceClosedSnapshots(totals, nextKey);
    financeDailyDays[financeDateKey] = normalizeFinanceDaily(financeDaily, financeDateKey);

    const existingNext = financeDailyDays[nextKey] ? normalizeFinanceDaily(financeDailyDays[nextKey], nextKey) : null;
    if (!existingNext || existingNext.isClosed) {
      financeDailyDays[nextKey] = buildFinanceNextDay(nextKey, totals.newCarriedAmount, financeDaily.pending);
    } else {
      existingNext.carriedAmountOverride = totals.newCarriedAmount;
      existingNext.pending = normalizeRows(financeDaily.pending);
      existingNext.updatedAt = new Date().toISOString();
      financeDailyDays[nextKey] = normalizeFinanceDaily(existingNext, nextKey);
    }
...
(function payrollAdvancesModule(){
  ...
  function employeeCellForPayroll(row){
    const current = getEmployeeById(row.employeeId);
    const employee = current || row.employee || {};
    const name = employee.name || 'موظف';
    const initials = employee.initials || (name || 'م').slice(0,1);
    const color = employee.color || 'teal';
    const avatarMarkup = `<div class="avatar avatar-${esc(color)}">${esc(initials)}</div>`;
    return `<div class="employee-cell payroll-name-only-cell">${avatarMarkup}<div><strong class="employee-name">${esc(name)}</strong></div></div>`;
  }
  ...
  function renderPayrollWithAdvances(){
    ...
    if (body) {
      ...
      const head = body.closest('table')?.querySelector('thead tr'); if (head) head.innerHTML = `<th>الموظف</th><th>الراتب الأساسي</th><th>البدلات</th><th>التأمينات</th><th>حسم الغياب</th><th>السلفيات</th><th>صافي الراتب</th><th>المحول للموظف</th><th>المطلوب من الموظف</th><th>سحب البطاقة</th><th>الحالة</th>`;
    }
  ...
  function payrollReportMarkupForPrint(key = monthKey()){
    ...
    const tableRows = rows.map(row => {
      const line = row.line || {};
      const requiredText = Number(line.required || 0) > 0 ? money(line.required) : '';
      return `<tr class="${line.cardWithdraw ? 'payroll-row-card-withdraw' : ''}"><td class="payroll-report-employee">${line.cardWithdraw ? `<span class="payroll-risk-marker"></span><span class="payroll-risk-name">${esc(payrollPlainEmployeeName(row))}</span>` : esc(payrollPlainEmployeeName(row))}</td><td class="payroll-base-amount">${money(line.baseSalary)}</td><td class="payroll-allowance-amount">${money(line.allowance)}</td><td class="payroll-deduction-amount">${money(line.insuranceDeduction)}</td><td class="payroll-deduction-amount">${money(line.absenceDeduction)}</td><td class="payroll-deduction-amount">${money(line.advanceDeduction)}</td><td class="payroll-net-amount">${money(line.net)}</td><td class="payroll-transfer-cell">${money(line.transfer)}</td><td class="payroll-required-cell">${requiredText}</td><td class="payroll-card-withdraw-cell">${line.cardWithdraw ? 'نعم' : ''}</td><td><span class="status-badge">${savedRun ? 'مصروف' : 'جاهز للصرف'}</span></td></tr>`;
    }).join('');
...
/* =========================================================
   Final permissions employee-user link resolver
   - Uses the same linked user data used by Users Management.
   - Matches employee links by id, employee number, national id, or embedded employee object.
   - Keeps the permissions screen on employees, not raw users.
   ========================================================= */
(function finalPermissionsEmployeeUserLinkResolver(){
  ...
  const isAdminUser = () => String(authProfile?.role || '').trim() === 'admin';
  const employeesList = () => (Array.isArray(employees) ? employees : []).filter((employee) => employee && employee.id && employee.status !== 'terminated');
  const matrix = () => window.employeePermissionMatrix || {};
  const permissionGroups = () => Array.isArray(matrix().groups) ? matrix().groups : [];
  const permissionKeys = () => permissionGroups().flatMap((group) => Array.isArray(group.items) ? group.items.map((item) => item[0]) : []);
  const normalizePerms = (permissions, role = 'employee') => typeof matrix().normalizePermissions === 'function'
    ? matrix().normalizePermissions(permissions, role)
    : Object.fromEntries(permissionKeys().map((key) => [key, Boolean(permissions && permissions[key]) ]));

  function compactArabicName(value){
    return norm(value).replace(/\s+/g, ' ').replace(/[\u0640]/g, '').trim();
  }

  function employeeName(employee){
    return compactArabicName(employee?.name || [employee?.firstName, employee?.fatherName, employee?.grandName, employee?.familyName].filter(Boolean).join(' '));
  }
  ...
  function profileEmployeeRawValues(profile){
    return [
      profile?.employee_id,
      profile?.employeeId,
      profile?.linked_employee_id,
      ...
      profile?.linkedEmployee?.id,
      profile?.linkedEmployee?.employeeNumber
    ].map(norm).filter(Boolean);
  }

  function profileEmployeeName(profile){
    return compactArabicName(
      profile?.employee_name ||
      profile?.employeeName ||
      profile?.linked_employee_name ||
      profile?.linkedEmployeeName ||
      profile?.employee?.name ||
      profile?.linkedEmployee?.name ||
      ''
    );
  }

  function profileMatchesEmployee(profile, employee){
    if (!profile || !employee) return false;
    const empKeys = employeeKeys(employee);
    if (profileEmployeeRawValues(profile).some((value) => empKeys.has(value))) return true;
    const pName = profileEmployeeName(profile);
    const eName = employeeName(employee);
    return Boolean(pName && eName && pName === eName);
  }

  function canonicalEmployeeIdForProfile(profile){
    const match = employeesList().find((employee) => profileMatchesEmployee(profile, employee));
  ...
  function mergeProfileRows(rows){
    ...
    function keysFor(row){
      return [row?.id, row?.user_id, normLower(row?.email)].map(norm).filter(Boolean);
    }
    function put(row){
      if (!row || typeof row !== 'object') return;
      const keys = keysFor(row);
      let existingKey = keys.find((key) => byKey.has(key));
      let merged = existingKey ? { ...byKey.get(existingKey), ...row } : { ...row };
      const canonical = canonicalEmployeeIdForProfile(merged) || canonicalEmployeeIdForProfile(row);
      if (canonical) {
        merged.employee_id = canonical;
        merged.employeeId = canonical;
      }
      keysFor(merged).forEach((key) => byKey.set(key, merged));
    }
    input.forEach(put);
    return [...new Set([...byKey.values()])];
  }

  async function loadLinkedProfilesForPermissions(){
    if (!isAdminUser()) return [];
    const before = Array.isArray(appUserProfilesCache) ? [...appUserProfilesCache] : [];
    let rows = [...before];
    ...
    try {
      if (supabaseClient?.functions?.invoke) {
        ...
        if (error) throw error;
        if (data?.error) throw new Error(data.error);
        if (Array.isArray(data?.users)) rows = rows.concat(data.users);
      }
    } catch (error) {
      console.warn('تعذر تحميل المستخدمين من دالة الإدارة', error);
    }
    ...
    try {
      ...
      console.warn('تعذر تحميل app_user_profiles مباشرة', error);
    }

    const merged = mergeProfileRows(rows).map((row) => {
      const role = String(row?.role || 'employee') === 'admin' ? 'admin' : 'employee';
      const canonical = canonicalEmployeeIdForProfile(row);
      return {
        ...row,
        role,
        permissions: normalizePerms(row?.permissions, role),
        employee_id: canonical || row?.employee_id || row?.employeeId || '',
        employeeId: canonical || row?.employeeId || row?.employee_id || ''
      };
    });
    appUserProfilesCache = merged;
    return merged;
  }

  function profileForEmployeeResolved(employee){
    return (Array.isArray(appUserProfilesCache) ? appUserProfilesCache : [])
      .find((profile) => String(profile?.role || 'employee') !== 'admin' && profileMatchesEmployee(profile, employee)) || null;
  }

  function permissionsEditorHtml(profile){
    const normalized = normalizePerms(profile?.permissions, profile?.role || 'employee');
    const groups = permissionGroups();
    if (!groups.length) {
      return `<div class="security-editor-empty"><strong>تعذر تحميل قائمة الصلاحيات</strong><p>أعد تحميل الصفحة ثم افتح شاشة الصلاحيات مرة أخرى.</p></div>`;
    }
    return `<div class="security-linked-employee-note"><strong>الحساب المرتبط:</strong> ${esc(profile?.full_name || profile?.email || '')} — <span dir="ltr">${esc(profile?.email || '')}</span></div>
      <div class="security-permissions-grid">${groups.map((group) => `
        <section class="security-permission-card">
          <div class="security-permission-head"><strong>${esc(group.title)}</strong><button type="button" class="text-btn" data-resolved-security-group="${esc(group.id)}">تحديد الكل</button></div>
          <div class="security-permission-list">
            ${(group.items || []).map(([key, label]) => `<label class="security-permission-row"><span>${esc(label)}</span><input type="checkbox" data-resolved-security-permission="${esc(key)}" ${normalized[key] ? 'checked' : ''}></label>`).join('')}
          </div>
        </section>`).join('')}</div>`;
  }

  function ensureResolvedPermissionsPanel(){
    const panel = document.querySelector('[data-settings-panel="permissions"]');
    if (!panel) return;
    panel.innerHTML = `
      <div class="panel-head"><div><h3>الصلاحيات والأمان</h3><p>اختر الموظف المرتبط بحساب مستخدم، ثم حدد الصلاحيات المسموحة له داخل النظام.</p></div></div>
      <div class="security-layout permissions-employee-picker-layout">
        <article class="settings-placeholder-card security-user-picker"><span data-icon="shield"></span><div><strong>تحديد الموظف</strong><p>تظهر هنا ملفات الموظفين. حفظ الصلاحيات يتم على حساب المستخدم المرتبط بالموظف.</p></div></article>
        <div class="security-controls"><select id="resolvedSecurityEmployeeSelect"><option value="">اختر موظفًا...</option></select><button type="button" class="secondary-btn" id="reloadResolvedSecurityEmployees"><span data-icon="refresh"></span>تحديث الموظفين</button><button type="button" class="primary-btn" id="saveResolvedEmployeeSecurityPermissions"><span data-icon="check"></span>حفظ الصلاحيات</button></div>
        <div id="resolvedEmployeeSecurityPermissionsEditor" class="security-editor-empty"><strong>اختر موظفًا من القائمة</strong><p>بعد الاختيار ستظهر الصلاحيات إذا كان الموظف مرتبطًا بحساب مستخدم.</p></div>
      </div>`;
    if (typeof hydrateIcons === 'function') hydrateIcons(panel);
  }

  async function loadResolvedSecurityEmployees(selectedId = ''){
    const select = document.querySelector('#resolvedSecurityEmployeeSelect');
    if (!select) return;
    select.disabled = false;
    select.innerHTML = `<option value="">جاري تحميل الموظفين...</option>`;
    await loadLinkedProfilesForPermissions();
    const list = employeesList();
    select.innerHTML = `<option value="">اختر موظفًا...</option>` + list.map((employee) => {
      const profile = profileForEmployeeResolved(employee);
      const suffix = profile ? ` - ${profile.email || profile.full_name || 'حساب مستخدم'}` : ' - غير مربوط بحساب';
      return `<option value="${esc(employee.id)}" ${String(employee.id) === String(selectedId) ? 'selected' : ''}>${esc(employeeName(employee) || employee.name || '')}${employee.employeeNumber ? ` - ${esc(employee.employeeNumber)}` : ''}${esc(suffix)}</option>`;
    }).join('');
    if (!list.length) select.innerHTML = `<option value="">لا يوجد موظفون</option>`;
    if (selectedId) select.value = selectedId;
  }

  function renderResolvedSelectedEmployeePermissions(){
    const employeeId = document.querySelector('#resolvedSecurityEmployeeSelect')?.value || '';
    const editor = document.querySelector('#resolvedEmployeeSecurityPermissionsEditor');
    if (!editor) return;
    const employee = employeesList().find((item) => String(item.id) === String(employeeId));
    const profile = profileForEmployeeResolved(employee);
    if (!employeeId || !employee) {
      editor.className = 'security-editor-empty';
      editor.innerHTML = `<strong>اختر موظفًا من القائمة</strong><p>بعد الاختيار ستظهر الصلاحيات إذا كان الموظف مرتبطًا بحساب مستخدم.</p>`;
    ...
    if (!profile) {
      editor.className = 'security-editor-empty';
      editor.innerHTML = `<strong>${esc(employeeName(employee) || employee.name || '')} غير مربوط بحساب مستخدم</strong><p>اربط الموظف أولًا من شاشة إدارة المستخدمين، ثم ارجع لتحديد الصلاحيات.</p>`;
      return;
    }
    editor.className = '';
    editor.innerHTML = permissionsEditorHtml(profile);
  ...
  async function saveResolvedEmployeePermissions(){
    const employeeId = document.querySelector('#resolvedSecurityEmployeeSelect')?.value || '';
    const employee = employeesList().find((item) => String(item.id) === String(employeeId));
    const profile = profileForEmployeeResolved(employee);
    if (!employeeId || !employee) return showToast('اختر موظفًا أولًا');
    if (!profile) return showToast('هذا الموظف غير مربوط بحساب مستخدم');
    const permissions = {};
    permissionKeys().forEach((key) => {
      permissions[key] = Boolean(document.querySelector(`[data-resolved-security-permission="${CSS.escape(key)}"]`)?.checked);
    });
    try {
      const payload = {
        action: 'update-user',
      ...
      profile.employee_id = employeeId;
      profile.employeeId = employeeId;
      profile.permissions = normalizePerms(permissions, 'employee');
      showToast('تم حفظ صلاحيات الموظف');
    } catch (error) {
      console.error(error);
      showToast('تعذر حفظ الصلاحيات');
  ...
  function openResolvedPermissionsPanel(selectedId = ''){
    ensureResolvedPermissionsPanel();
    loadResolvedSecurityEmployees(selectedId).then(() => {
      if (selectedId) renderResolvedSelectedEmployeePermissions();
    }).catch((error) => {
      console.error(error);
      const select = document.querySelector('#resolvedSecurityEmployeeSelect');
      if (select) select.innerHTML = `<option value="">تعذر تحميل الموظفين</option>`;
      showToast('تعذر تحميل موظفي الصلاحيات');
    });
  }
  ...
  if (previousSwitchSettingsSection) {
    switchSettingsSection = function(section){
      previousSwitchSettingsSection(section);
      if (section === 'permissions') openResolvedPermissionsPanel();
    };
  }

  document.addEventListener('change', (event) => {
    if (event.target?.id === 'resolvedSecurityEmployeeSelect') renderResolvedSelectedEmployeePermissions();
  }, true);

  document.addEventListener('click', (event) => {
    if (event.target.closest('#reloadResolvedSecurityEmployees')) {
      event.preventDefault();
      event.stopPropagation();
      openResolvedPermissionsPanel(document.querySelector('#resolvedSecurityEmployeeSelect')?.value || '');
      return;
    }
    if (event.target.closest('#saveResolvedEmployeeSecurityPermissions')) {
      event.preventDefault();
      event.stopPropagation();
      saveResolvedEmployeePermissions();
      return;
    }
    const group = event.target.closest('[data-resolved-security-group]');
    if (group) {
      event.preventDefault();
      event.stopPropagation();
      const card = group.closest('.security-permission-card');
      const boxes = [...(card?.querySelectorAll('[data-resolved-security-permission]') || [])];
      const shouldCheck = boxes.some((box) => !box.checked);
      boxes.forEach((box) => { box.checked = shouldCheck; });
    }
  }, true);
  ...
  setTimeout(() => {
    const permissionsPanel = document.querySelector('[data-settings-panel="permissions"]');
    if (permissionsPanel && (permissionsPanel.offsetParent !== null || permissionsPanel.classList.contains('active'))) {
      openResolvedPermissionsPanel();
    }
  }, 300);
})();
...
/* =========================================================
   FINAL HOTFIX - user employee link + edge error handling
   =========================================================
   - Permissions screen now resolves linked profiles exactly from the visible
     users management cache and app_user_profiles data.
   - Admin-linked employees are considered linked for display/selection.
   - Edge Function errors are parsed so the real Supabase reason appears.
   - Existing-user updates fall back to direct app_user_profiles update when
     the Edge Function is unavailable, because updating the profile row does
     not require creating an Auth user.
*/
(function finalUserEmployeeLinkAndEdgeErrorHotfix(){
  const esc = (value) => typeof escapeHtml === 'function'
    ? escapeHtml(value ?? '')
    : String(value ?? '').replace(/[&<>\"]/g, (m) => ({'&':'&amp;','<':'&lt;','>':'&gt;','\"':'&quot;'}[m] || m));
  const text = (value) => String(value ?? '').trim();
  const lower = (value) => text(value).toLowerCase();
  const cleanName = (value) => text(value).replace(/[\u0640]/g, '').replace(/\s+/g, ' ').trim();
  const employeesList = () => (Array.isArray(employees) ? employees : []).filter((e) => e && e.id && e.status !== 'terminated');
  const employeeName = (e) => cleanName(e?.name || [e?.firstName, e?.fatherName, e?.grandName, e?.familyName].filter(Boolean).join(' '));
  const employeeKeys = (e) => new Set([
    e?.id, e?.employeeId, e?.employee_id, e?.employeeNumber, e?.employeeNo, e?.number, e?.code,
    e?.nationalId, e?.national_id, e?.identityNumber, e?.idNumber, e?.iqamaNumber, e?.residencyNumber
  ].map(text).filter(Boolean));
  const profileValues = (p) => [
    p?.employee_id, p?.employeeId, p?.linked_employee_id, p?.linkedEmployeeId,
    p?.employee_number, p?.employeeNumber, p?.employee_no, p?.employeeNo,
    p?.employee_code, p?.employeeCode, p?.national_id, p?.nationalId,
    p?.identity_number, p?.identityNumber, p?.employee?.id, p?.employee?.employee_id,
    p?.employee?.employeeId, p?.employee?.employeeNumber, p?.employee?.employee_number,
    p?.linkedEmployee?.id, p?.linkedEmployee?.employeeNumber
  ].map(text).filter(Boolean);
  const profileLinkedName = (p) => cleanName(p?.employee_name || p?.employeeName || p?.linked_employee_name || p?.linkedEmployeeName || p?.employee?.name || p?.linkedEmployee?.name || '');

  async function readFunctionError(error){
    const fallback = String(error?.message || error || 'تعذر الاتصال بدالة إدارة المستخدمين');
    try {
      const ctx = error?.context;
      if (ctx && typeof ctx.clone === 'function') {
        const body = await ctx.clone().json().catch(async () => ({ error: await ctx.clone().text() }));
  ...
  async function invokeAdminUsers(body){
    if (!supabaseClient?.functions?.invoke) throw new Error('Supabase غير متصل أو دالة المستخدمين غير متاحة');
  ...
  function profileMatchesEmployee(profile, employee){
    ...
    if (profileValues(profile).some((value) => keys.has(value))) return true;
    const pName = profileLinkedName(profile);
    const eName = employeeName(employee);
    return Boolean(pName && eName && pName === eName);
  }

  function canonicalEmployeeId(profile){
    const direct = text(profile?.employee_id || profile?.employeeId || profile?.linked_employee_id || profile?.linkedEmployeeId);
    if (direct && employeesList().some((e) => text(e.id) === direct)) return direct;
    const byAnyKey = employeesList().find((employee) => profileValues(profile).some((value) => employeeKeys(employee).has(value)));
    if (byAnyKey?.id) return text(byAnyKey.id);
    const pName = profileLinkedName(profile);
    if (pName) {
      const byName = employeesList().find((employee) => employeeName(employee) === pName);
      if (byName?.id) return text(byName.id);
    }
    return direct || '';
  }
  ...
  function mergeProfiles(rows){
    ...
    (Array.isArray(rows) ? rows : []).forEach((row) => {
      if (!row || typeof row !== 'object') return;
      const keys = [row.id, row.user_id, lower(row.email)].map(text).filter(Boolean);
      const existingKey = keys.find((key) => byEmailOrId.has(key));
      const merged = existingKey ? { ...byEmailOrId.get(existingKey), ...row } : { ...row };
      const canonical = canonicalEmployeeId(merged);
      if (canonical) { merged.employee_id = canonical; merged.employeeId = canonical; }
      keys.forEach((key) => byEmailOrId.set(key, merged));
    });
    return Array.from(new Set(byEmailOrId.values()));
  }

  async function loadProfilesForPermissions(){
    const rows = [];
    if (Array.isArray(appUserProfilesCache)) rows.push(...appUserProfilesCache);
    try {
      const data = await invokeAdminUsers({ action: 'list-users' });
      if (Array.isArray(data?.users)) rows.push(...data.users);
    } catch (error) {
      console.warn('admin-create-user list-users failed, using direct table/cache', error);
    }
    try {
      if (supabaseClient?.from) {
        const { data, error } = await supabaseClient
      ...
      console.warn('direct app_user_profiles load failed', error);
    }
    const merged = mergeProfiles(rows).map((row) => {
      const role = String(row?.role || 'employee') === 'admin' ? 'admin' : 'employee';
      const id = canonicalEmployeeId(row);
      return { ...row, role, employee_id: id || row.employee_id || row.employeeId || '', employeeId: id || row.employeeId || row.employee_id || '' };
    });
    appUserProfilesCache = merged;
    return merged;
  }

  function profileForEmployee(employee){
    return (Array.isArray(appUserProfilesCache) ? appUserProfilesCache : [])
      .find((profile) => profileMatchesEmployee(profile, employee)) || null;
  }
  ...
  function permissionKeys(){ return permissionGroups().flatMap((g) => Array.isArray(g.items) ? g.items.map((item) => item[0]) : []); }
  function normalizePermissionObject(permissions, role){
    const m = window.employeePermissionMatrix || {};
    if (typeof m.normalizePermissions === 'function') return m.normalizePermissions(permissions, role);
    if (typeof normalizePermissions === 'function') return normalizePermissions(permissions, role);
    const result = {};
    permissionKeys().forEach((key) => result[key] = Boolean(permissions && permissions[key]));
    return result;
  }
  ...
  function ensurePanel(){
    ...
    panel.innerHTML = `
      ...
      <div class="security-layout permissions-employee-picker-layout">
        <article class="settings-placeholder-card security-user-picker"><span data-icon="shield"></span><div><strong>تحديد الموظف</strong><p>تظهر هنا ملفات الموظفين. حفظ الصلاحيات يتم على حساب المستخدم المرتبط بالموظف.</p></div></article>
        <div class="security-controls"><select id="fixedSecurityEmployeeSelect"><option value="">اختر موظفًا...</option></select><button type="button" class="secondary-btn" id="reloadFixedSecurityEmployees"><span data-icon="refresh"></span>تحديث الموظفين</button><button type="button" class="primary-btn" id="saveFixedEmployeeSecurityPermissions"><span data-icon="check"></span>حفظ الصلاحيات</button></div>
        <div id="fixedEmployeeSecurityPermissionsEditor" class="security-editor-empty"><strong>اختر موظفًا من القائمة</strong><p>بعد الاختيار ستظهر الصلاحيات إذا كان الموظف مرتبطًا بحساب مستخدم.</p></div>
      </div>`;
    if (typeof hydrateIcons === 'function') hydrateIcons(panel);
  }

  async function loadEmployeeSelect(selected = ''){
    const select = document.querySelector('#fixedSecurityEmployeeSelect');
    if (!select) return;
    select.disabled = false;
    select.innerHTML = '<option value="">جاري تحميل الموظفين...</option>';
    await loadProfilesForPermissions();
    const options = employeesList().map((employee) => {
      const profile = profileForEmployee(employee);
      const linked = profile ? ` - ${profile.email || profile.full_name || 'حساب مستخدم'}` : ' - غير مربوط بحساب';
      const num = employee.employeeNumber ? ` - ${employee.employeeNumber}` : '';
      return `<option value="${esc(employee.id)}" ${text(employee.id) === text(selected) ? 'selected' : ''}>${esc(employeeName(employee) || employee.name || '')}${esc(num)}${esc(linked)}</option>`;
    }).join('');
    select.innerHTML = '<option value="">اختر موظفًا...</option>' + options;
    if (!employeesList().length) select.innerHTML = '<option value="">لا يوجد موظفون</option>';
    if (selected) select.value = selected;
  }

  function renderEditor(){
    const id = document.querySelector('#fixedSecurityEmployeeSelect')?.value || '';
    const editor = document.querySelector('#fixedEmployeeSecurityPermissionsEditor');
    if (!editor) return;
    const employee = employeesList().find((item) => text(item.id) === text(id));
    const profile = profileForEmployee(employee);
    if (!id || !employee) {
      editor.className = 'security-editor-empty';
      editor.innerHTML = '<strong>اختر موظفًا من القائمة</strong><p>بعد الاختيار ستظهر الصلاحيات إذا كان الموظف مرتبطًا بحساب مستخدم.</p>';
    ...
    const normalized = normalizePermissionObject(profile.permissions, role);
    const groups = permissionGroups();
    const adminNote = role === 'admin' ? '<p class="security-linked-employee-note"><strong>تنبيه:</strong> هذا الحساب مدير نظام، ويملك كامل الصلاحيات فعليًا. يمكنك تعديل الربط من إدارة المستخدمين.</p>' : '';
    editor.className = '';
    editor.innerHTML = `${adminNote}<div class="security-linked-employee-note"><strong>الحساب المرتبط:</strong> ${esc(profile.full_name || profile.email || '')} — <span dir="ltr">${esc(profile.email || '')}</span></div>
      <div class="security-permissions-grid">${groups.map((group) => `
        <section class="security-permission-card">
          <div class="security-permission-head"><strong>${esc(group.title)}</strong><button type="button" class="text-btn" data-fixed-security-group="${esc(group.id)}">تحديد الكل</button></div>
          <div class="security-permission-list">
            ${(group.items || []).map(([key, label]) => `<label class="security-permission-row"><span>${esc(label)}</span><input type="checkbox" data-fixed-security-permission="${esc(key)}" ${normalized[key] ? 'checked' : ''}></label>`).join('')}
          </div>
        </section>`).join('')}</div>`;
  }

  async function savePermissions(){
    const employeeId = document.querySelector('#fixedSecurityEmployeeSelect')?.value || '';
    const employee = employeesList().find((item) => text(item.id) === text(employeeId));
    const profile = profileForEmployee(employee);
    if (!employeeId || !employee) return showToast('اختر موظفًا أولًا');
    if (!profile) return showToast('هذا الموظف غير مربوط بحساب مستخدم');
    const permissions = {};
    permissionKeys().forEach((key) => permissions[key] = Boolean(document.querySelector(`[data-fixed-security-permission="${CSS.escape(key)}"]`)?.checked));
    try {
      try {
        const data = await invokeAdminUsers({ action: 'update-user', id: profile.id, email: profile.email, fullName: profile.full_name, role: profile.role || 'employee', isActive: profile.is_active, employeeId, employee_id: employeeId, permissions });
        if (data?.user) Object.assign(profile, data.user);
      } catch (edgeError) {
        if (!supabaseClient?.from) throw edgeError;
        const { data, error } = await supabaseClient.from('app_user_profiles')
          .update({ employee_id: employeeId, permissions, updated_at: new Date().toISOString() })
          .eq('id', profile.id)
          .select('id,user_id,full_name,email,role,is_active,employee_id,permissions,created_at,updated_at')
          .single();
      ...
      profile.employee_id = employeeId;
      profile.employeeId = employeeId;
      profile.permissions = normalizePermissionObject(permissions, profile.role || 'employee');
      await loadEmployeeSelect(employeeId);
      renderEditor();
      showToast('تم حفظ صلاحيات الموظف');
    } catch (error) {
      console.error(error);
      showToast(String(error?.message || 'تعذر حفظ الصلاحيات').slice(0, 160));
    }
  }

  async function openPanel(selected = ''){
    ensurePanel();
    try { await loadEmployeeSelect(selected); renderEditor(); }
    catch (error) { console.error(error); showToast('تعذر تحميل موظفي الصلاحيات'); }
  }

  const oldRenderPermissionsPreview = typeof renderPermissionsPreview === 'function' ? renderPermissionsPreview : null;
  renderPermissionsPreview = function(){ try { oldRenderPermissionsPreview?.(); } catch (_) {} openPanel(); };
  const oldSwitchSettingsSection = typeof switchSettingsSection === 'function' ? switchSettingsSection : null;
  if (oldSwitchSettingsSection) {
    switchSettingsSection = function(section){
      oldSwitchSettingsSection(section);
      if (section === 'permissions') openPanel();
    };
  }

  document.addEventListener('change', (event) => {
    if (event.target?.id === 'fixedSecurityEmployeeSelect') renderEditor();
  }, true);
  document.addEventListener('click', (event) => {
    if (event.target.closest('#reloadFixedSecurityEmployees')) { event.preventDefault(); event.stopPropagation(); openPanel(document.querySelector('#fixedSecurityEmployeeSelect')?.value || ''); return; }
    if (event.target.closest('#saveFixedEmployeeSecurityPermissions')) { event.preventDefault(); event.stopPropagation(); savePermissions(); return; }
    const group = event.target.closest('[data-fixed-security-group]');
    if (group) {
      event.preventDefault(); event.stopPropagation();
      const boxes = Array.from(group.closest('.security-permission-card')?.querySelectorAll('[data-fixed-security-permission]') || []);
      const shouldCheck = boxes.some((box) => !box.checked);
      boxes.forEach((box) => box.checked = shouldCheck);
    }
  }, true);

  // Improve create/update user form feedback and direct update fallback.
  const oldSaveUserProfile = typeof saveUserProfileFromForm === 'function' ? saveUserProfileFromForm : null;
  if (oldSaveUserProfile) {
    saveUserProfileFromForm = async function(form){
      const profileId = form.elements.profileId?.value?.trim() || '';
      const employeeId = form.elements.employeeId?.value || null;
      try {
        await oldSaveUserProfile(form);
        // If editing an existing user and old handler succeeded, ensure cache/table link uses employee_id.
        if (profileId && supabaseClient?.from) {
          await supabaseClient.from('app_user_profiles').update({ employee_id: employeeId, updated_at: new Date().toISOString() }).eq('id', profileId);
        }
      } catch (error) {
        console.error(error);
        showToast(String(error?.message || 'تعذر حفظ المستخدم').slice(0, 160));
      }
    };
  }
...
(function financeDailyTablesStep3(){
  ...
  function financeMonthKey(dateKey = financeDateKey) {
    return String(dateKey || getFinanceDateKey()).slice(0, 7);
  }

  function calculateFinanceAdvancesTotal() {
    const month = financeMonthKey();
    try {
      const rows = JSON.parse(localStorage.getItem("nawah-payroll-advances") || "[]");
      if (!Array.isArray(rows)) return 0;
      return rows.reduce((sum, item) => {
        if (!item || item.status !== "approved") return sum;
        const itemMonth = item.monthKey || String(item.date || "").slice(0, 7);
        if (itemMonth !== month) return sum;
  ...
  function financeAdvanceEmployeeCell(item) {
    try {
      const employee = typeof getEmployeeById === 'function' ? getEmployeeById(item.employeeId) : null;
      if (employee) {
        const name = employee.name || financeAdvanceEmployeeName(item);
        const initials = employee.initials || (name || 'م').slice(0,1);
        const color = employee.color || 'teal';
        return `<div class="employee-cell payroll-name-only-cell"><div class="avatar avatar-${escapeFinanceText(color)}">${escapeFinanceText(initials)}</div><div><strong class="employee-name">${escapeFinanceText(name)}</strong></div></div>`;
      }
    } catch (_) {}
  ...
  function calculateTotals() {
    ...
    const advancesTotal = calculateFinanceAdvancesTotal();
...
/*
   Final decisive fix v3:
   - Stable linked employee photo in the topbar without flicker.
   - Direct attachment download through Blob/Supabase Storage.download, no popup dependency.
   - Employee attachment controls keep the download icon beside the attach control.
*/
(function(){
  const htmlEscape = (value) => String(value ?? '').replace(/[&<>"']/g, (ch) => ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[ch]));
  const safeIcon = (name) => {
    try { return typeof iconSvg === 'function' ? iconSvg(name) : '<span data-icon="download"></span>'; }
  ...
  function currentHeaderName(){
    ...
    return employee?.name || authProfile?.full_name || authUser?.user_metadata?.full_name || authUser?.email?.split('@')[0] || 'مستخدم';
  }

  const headerPhotoCache = new Map();
  async function getHeaderPhotoUrl(photoId){
    if (!photoId) return '';
    const key = String(photoId);
    if (headerPhotoCache.has(key)) return headerPhotoCache.get(key);
    let url = '';
    try { url = typeof attachmentUrl === 'function' ? await attachmentUrl(key) : ''; } catch (_) { url = ''; }
    if (url) headerPhotoCache.set(key, url);
    return url;
  }

  function setHeaderFallback(mark, name){
    if (!mark) return;
    const initial = String(name || 'م').trim().charAt(0) || 'م';
    if (mark.dataset.headerMode === 'initial' && mark.textContent === initial) return;
    mark.dataset.headerMode = 'initial';
    mark.dataset.headerPhotoId = '';
    mark.classList.remove('has-employee-photo');
    mark.textContent = initial;
  }

  async function applyStableTopbarPhoto(){
    const mark = document.querySelector('.topbar-user-mark');
    if (!mark) return;
    const employee = linkedEmployeeForHeader();
    ...
    if (photoId) {
      const key = String(photoId);
      if (mark.dataset.headerMode === 'photo' && mark.dataset.headerPhotoId === key && mark.querySelector('img')) return;
      const url = await getHeaderPhotoUrl(key);
      if (url) {
        mark.dataset.headerMode = 'photo';
        mark.dataset.headerPhotoId = key;
        mark.classList.add('has-employee-photo');
        mark.innerHTML = `<img src="${htmlEscape(url)}" alt="" />`;
        return;
      }
      setHeaderFallback(mark, name);
      return;
    }

    if (legacyPhoto) {
      const key = String(legacyPhoto);
      if (mark.dataset.headerMode === 'legacy-photo' && mark.dataset.headerPhotoId === key && mark.querySelector('img')) return;
      mark.dataset.headerMode = 'legacy-photo';
      mark.dataset.headerPhotoId = key;
      mark.classList.add('has-employee-photo');
      mark.innerHTML = `<img src="${htmlEscape(legacyPhoto)}" alt="" />`;
      return;
    }
  ...
  try { window.__applyStableTopbarPhoto = applyStableTopbarPhoto; } catch (_) {}

  function updateStableTopbarUser(){
    const name = currentHeaderName();
    const role = String(authProfile?.role || '').trim() === 'admin' ? 'مدير النظام' : 'موظف';
    const copy = document.querySelector('.topbar-user-copy');
    if (copy) copy.innerHTML = `<strong>${htmlEscape(name)}</strong><span>${htmlEscape(role)}</span>`;
    setHeaderFallback(document.querySelector('.topbar-user-mark'), name);
    applyStableTopbarPhoto();
    document.querySelectorAll('.dashboard-welcome-compact .welcome-copy h2').forEach((h) => { h.textContent = `صباح الخير، ${name}`; });
  }

  try { updateTopbarUser = updateStableTopbarUser; } catch (_) {}
  try { window.updateTopbarUser = updateStableTopbarUser; } catch (_) {}

  const attachmentName = (record, fallback = 'attachment') => record?.name || record?.file_name || record?.fileName || fallback;
  ...
  function ensureFileName(record){
    const raw = attachmentName(record, 'attachment');
    if (/\.[a-z0-9]{2,8}$/i.test(raw)) return raw;
    return raw + extensionFromType(record?.type || record?.file_type || '');
  }

  async function blobFromAttachmentRecord(record, id){
    if (!record) return null;
    if (record.blob) return record.blob;
    if (record.attachmentData || record.dataUrl) {
      const res = await fetch(record.attachmentData || record.dataUrl);
  ...
  async function directDownloadAttachment(id){
    ...
    try {
      const blob = await blobFromAttachmentRecord(record, id);
      if (!blob) { try { showToast('لا يوجد مرفق محفوظ'); } catch (_) {} return; }
      const url = URL.createObjectURL(blob);
      const link = document.createElement('a');
      link.href = url;
      link.download = ensureFileName(record || {});
      link.style.display = 'none';
      document.body.appendChild(link);
      link.click();
      link.remove();
  ...
  try { window.directDownloadAttachment = directDownloadAttachment; } catch (_) {}
  try { openAttachment = directDownloadAttachment; } catch (_) {}

  function downloadButtonHtml(attachmentId){
    return `<button type="button" class="attachment-view-btn attachment-download-btn employee-attachment-download-inline" data-view-attachment="${htmlEscape(attachmentId)}" title="تحميل المرفق" aria-label="تحميل المرفق">${safeIcon('download')}</button>`;
  }

  try {
    attachmentControlHtml = function(kind, index, attachmentId, label){
      const id = attachmentId || '';
      const control = `<span class="compact-file-control ${id ? 'has-file' : ''}">${safeIcon('file')}<span>${id ? 'تم الإرفاق' : 'إرفاق'}</span><input type="file" data-${htmlEscape(kind)}-attachment="${htmlEscape(index)}" accept="image/*,.pdf" /></span>`;
      return `<span class="employee-attachment-inline-wrap">${control}${id ? downloadButtonHtml(id) : ''}</span>`;
    };
    window.attachmentControlHtml = attachmentControlHtml;
  } catch (_) {}

  function normalizeFinalAttachmentButtons(root = document){
    try {
      root.querySelectorAll('[data-view-attachment], [data-view-single-attachment]').forEach((button) => {
        button.classList.add('attachment-download-btn');
        button.setAttribute('title', 'تحميل المرفق');
        button.setAttribute('aria-label', 'تحميل المرفق');
        button.innerHTML = safeIcon('download');
      });
      if (typeof hydrateIcons === 'function') hydrateIcons(root);
    } catch (_) {}
  }
  ...
  document.addEventListener('click', async function(event){
    ...
    if (direct || single) {
      ...
      event.stopPropagation();
      event.stopImmediatePropagation();
      await directDownloadAttachment(direct?.dataset.viewAttachment || single?.dataset.attachmentId || '');
    }
  }, true);
  ...
  document.addEventListener('change', async function(event){
    const target = event.target;
    if (!target) return;
    const isEmployeeAttachment = target.matches('[data-passport-attachment], [data-bank-certificate-attachment], [data-bank-approval-attachment], [data-document-attachment], [data-single-attachment]');
    if (!isEmployeeAttachment) return;
    setTimeout(() => {
      try { normalizeFinalAttachmentButtons(document); } catch (_) {}
      try { if (typeof queueCloudStateSave === 'function') queueCloudStateSave(); } catch (_) {}
    }, 0);
  }, true);

  const prevRenderAllFinal = typeof renderAll === 'function' ? renderAll : null;
  if (prevRenderAllFinal && !prevRenderAllFinal.__finalAttachmentAndHeaderFix) {
    const wrapped = function(){
      const result = prevRenderAllFinal.apply(this, arguments);
      setTimeout(() => { normalizeFinalAttachmentButtons(document); updateStableTopbarUser(); }, 0);
      return result;
    };
    wrapped.__finalAttachmentAndHeaderFix = true;
    try { renderAll = wrapped; } catch (_) {}
  }

  const mo = new MutationObserver((mutations) => {
    let touched = false;
    for (const m of mutations) { if (m.addedNodes?.length) { touched = true; break; } }
    if (touched) setTimeout(() => { normalizeFinalAttachmentButtons(document); applyStableTopbarPhoto(); }, 0);
  });
  try { mo.observe(document.documentElement, { childList: true, subtree: true }); } catch (_) {}
  document.addEventListener('DOMContentLoaded', () => setTimeout(() => { normalizeFinalAttachmentButtons(document); updateStableTopbarUser(); }, 60));
